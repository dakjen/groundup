...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'join') {
      const { name, email, phone, learn, pain, budget, source } = req.body;
      ...
      const [entry] = await sql`
        INSERT INTO waitlist (name, email, phone, learn, reason, budget, source, created_at)
        VALUES (${String(name).trim()}, ${cleanEmail}, ${cleanPhone}, ${cleanLearn}, ${cleanPain}, ${budget}, ${cleanSource}, NOW())
